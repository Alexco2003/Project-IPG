...
void Lab06::Init()
{
    ...
    CreateShader("Bonus",
        "src/lab/lab06/shaders/Bonus.VS.glsl",
        "src/lab/lab06/shaders/Bonus.FS.glsl"
	);
}
...
Mesh *Lab06::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
    ...
    glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, 44, (void*)12);
...
void Lab06::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderMesh(meshes["sphere"], shaders["Bonus"], model, GetSceneCamera(), viewport_space);
    }

    {
        ...
        RenderMesh(meshes["cube"], shaders["Bonus"], model, GetSceneCamera(), viewport_space);
    }
    ...
    const gfxc::Camera *camera, const transform2D::ViewportSpace &viewport_space)
{
	...
	location = glGetUniformLocation(shader->program, "a");
	glUniform1i(location, 10);

	location = glGetUniformLocation(shader->program, "b");
	glUniform3f(location, 1, 2, 3);

	location = glGetUniformLocation(shader->program, "c");
	glUniform2f(location, 1, 2);

    location = glGetUniformLocation(shader->program, "d");
    glUniform3i(location, 1, 2, 3);

	location = glGetUniformLocation(shader->program, "e");
    glm::vec3 e[100];
	glUniform3fv(location, 100, glm::value_ptr(e[0]));
