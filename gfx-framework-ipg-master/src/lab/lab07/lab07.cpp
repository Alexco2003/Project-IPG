...
void Lab07::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & model, const glm::vec3 &object_color)
{
	...
	int location = glGetUniformLocation(shader->program, "point_light_positions");
	...
	glUniform1fv(location, 10, &spot_light_angles[0]);
	...
	glUniform3fv(location, 1, glm::value_ptr(eye_position));
...
void Lab07::OnInputUpdate(float deltaTime, int mods)
{
...

    {
        ...
        if (window->KeyHold(GLFW_KEY_Y)) {
            glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(-30.0f) * deltaTime, glm::vec3(1, 0, 0));
        ...
        if (window->KeyHold(GLFW_KEY_H)) {
			glm::mat4 rotation = glm::rotate(glm::mat4(1.0f), glm::radians(-30.0f) * deltaTime, glm::vec3(0, 0, 1));
            light_direction = glm::mat3(rotation) * light_direction;
        }
